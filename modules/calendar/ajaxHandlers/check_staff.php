...
if ($auth->isLogin()) {
    ...
    if ($auth->checkAjax()) {
        // Блокируем назначение только если приказ не утверждён и это плановая проверка
        if (!$order_approved) {
            echo json_encode([
                'result' => false,
        ...
        if (!isset($_POST['executors'])) {
            ...
            // Для внеплановых — проверяем что учреждение выбрано
            if ($is_unplanned && $insId == 0) {
                $err++;
                $errStr[] = 'Укажите объект проверки';
                $errorFields[] = 'ins_select';
            }
            $is_head_count = 0;
