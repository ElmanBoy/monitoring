...
$rowId = is_array($_POST['params']) ? intval($_POST['params'][0]) : intval($_POST['params']);

$registry = $db->selectOne('checkitems', ' where id = ?', [$rowId]);
$blocks = $db->getRegistry('checkitems', ' where is_block = 1');
...
if ($busy != []) {
    ...
    <div class='pop_up drag' style='width: 60vw;'>
        <div class='title handle'>
            ...
            <div class='name'>Редактирование пункта чек-листа</div>
            <div class='button icon close'><span class='material-icons'>close</span></div>
        </div>
        <div class='pop_up_body'>
            <form class='ajaxFrm' id='item_edit' onsubmit='return false'>
                <input type='hidden' name='path' value='checklists/items'>
                <input type='hidden' name='is_block' value='<?=intval($registry->is_block)?>'>
                <div class='tab-panel' id='tab_main-panel'>
                    <div class='group'>
                        ...
                        <div class='item w_50 required'>
                            <div class='el_data'>
                                <label>Наименование пункта</label>
                                <textarea class='el_textarea' name='prop_name'><?= $registry->name ?></textarea>
                            </div>
                        </div>
                        <div class='item w_50 required'>
                            <div class='el_data'>
                                ...
                                <input required class='el_input' type='text' name='field_name' maxlength='60'
                                       value="<?= $registry->field_name ?>">
                            </div>
                        </div>
                        <div class='item w_50'>
                            <select data-label='Входит в блок' name='block_id'>
                                <option value='0'>Не входит в блоки</option>
                                <? foreach ($blocks['array'] as $id => $name) { ?>
                                    <option value="<?= $id ?>"<?= ($id == $registry->block_id ? ' selected' : '') ?>><?= $name ?></option>
                                <? } ?>
                            </select>
                        </div>
                        <?
                        if ($registry->is_block != 1) {
                            ?>
                            <div class='item w_50'>
                                <div class='el_data horizontal'>
                                    <label>Тип поля:</label>
                                    <div class='custom_checkbox'>
                                        <label class='container'>
                                            <span class='label-text'> Пункт чек-листа</span>
                                            <input type='radio' class='for_self' name='item_type' value='0'
                                                <?= $registry->item_type == '0' || $registry->item_type == null ? ' checked' : '' ?>>
                                            <span class='checkmark radio'></span>
                                        </label>
                                    </div>
                                    <div class='custom_checkbox'>
                                        <label class='container'>
                                            <span class='label-text'> Произвольное поле</span>
                                            <input type='radio' class='for_self' name='item_type' value='1'
                                                <?= $registry->item_type == '1' ? ' checked' : '' ?>>
                                            <span class='checkmark radio'></span>
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div class='item w_50 required'>
                                <select data-label='Тип ответа' name='field_types' required='required'>
                            ...
                            <div class='item w_100 field_option' id='html_value'>
                                <div class='el_data'>
                                    <label>Текст по умолчанию</label>
                                    <textarea name='default_value' class='html_editor el_input'><?= $registry->default_value ?></textarea>
                                </div>
                            </div>
                            <div class='item w_100 field_option' id='default_date'>
                                <div class='el_data' style="margin-top: 30px;">
                                    ...
                                    <div class='custom_checkbox'>
                                        <label class='container'>
                                            <input type='checkbox' name='curr_date' id='curr_date' tabindex='-1'
                                                   value='1'
                            ...
                            <div class='item w_100 field_option' id='default_time'>
                                <div class='el_data' style='margin-top: 30px;'>
                                    ...
                                    <div class='custom_checkbox'>
                                        <label class='container'>
                                            <input type='checkbox' name='curr_time' id='curr_time' tabindex='-1'
                                                   value='1'
                                ...
                                if (is_array($radio_values)) {
                                    for ($r = 1; $r < count($radio_values); $r++) {
                                        ...
                                        <div class='radio_button group'>
                                            <h5 class='item w_100 radio_number'>Кнопка №<?= $r ?></h5>
                                            <?
                                            if ($r > 1) {
                                                echo '<div class="button icon clear"><span class="material-icons">close</span></div>';
                                            }
                                            ?>
                                            <div class='item w_50'>
                                                <div class='el_data'>
                                    ...
                                    ?>
                                    <div class='radio_button group'>
                                        <h5 class='item w_100 radio_number'>Кнопка №1</h5>
                                        <div class='item w_50'>
                                            <div class='el_data'>
                                                <label>Название кнопки:</label>
                                                <input name='radio_label[]' class='el_input' type='text'>
                            ...
                            <div class='item w_100 field_option' id='fromdb'>
                                <div style="margin-top: -10px; margin-right: 50px;">
                                    ...
                                    <div class='custom_checkbox'>
                                        <label class='container'> Выпадающий список (одиночный выбор)
                                            <input type='radio' name='from_db_view' value='0'
                                                <?= ($registry->from_db_view == '0' || $registry->from_db_view == '' ? ' checked' : '') ?>>
                                            <span class="checkmark radio"></span></label>
                                    </div>
                                    <div class='custom_checkbox'>
                                    ...
                                    <div class="custom_checkbox">
                                        <label class="container"> Группа чек-кнопок
                                            <input type="radio" name="from_db_view" value="2"
                                                <?= ($registry->from_db_view == '2' ? ' checked' : '') ?>>
                                            <span class="checkmark radio"></span></label>
                                    </div>
                                    <div class='custom_checkbox'>
                                        <label class='container'> Группа радио-кнопок
                                ...
                                <div style="width: 100%; display: flex; flex-flow: column; gap: 20px;">
                                    <div class='el_data'>
                                    ...
                                    <div class='el_data'>
                                        <input type="hidden" name="selected_field"
                                               value="<?= $registry->from_db_text ?>">
    ...
    <script>
        ...
        tinymce.init({
            target: document.querySelector('.html_editor'),
            language: 'ru',
