...
var el_registry = {
    ...
    create_init: function(){
        ...
        $("#button_nav_create:not(.disabled)").off("click").on("click", function () {
            el_app.dialog_open("prop_create", "", "checklists/items");
        });
        $("#tbl_items td .button").off("click").on("click", function () {
            el_app.dialog_open("prop_create", $(this).closest("tr").data("id"), "checklists/items");
        });

        $("#button_nav_create_block:not(.disabled)").off("click").on("click", function () {
            el_app.dialog_open("block_create", "", "checklists/items");
        });

        $("#button_nav_delete").on("click", async function () {
            if (!$(this).hasClass("disabled")) {
                let ok = await confirm("Уверены, что хотите удалить эти пункты?");
                if (ok) {
                    $("form#item_delete").trigger("submit");
                }
            }
        });
        $("#button_nav_clone").on("click", async function () {
            if (!$(this).hasClass("disabled")) {
                $("form#item_delete").attr("id", "item_clone").trigger("submit").attr("id", "item_delete");
            }
        });
        ...
        $("#button_nav_checks").on("click", function (){
        ...
        $(".new_check").on("click", function (e){
            e.preventDefault();
            $(".pop_up_body .radio_button:last").clone().insertAfter(".pop_up_body .radio_button:last");
            $(".pop_up_body .radio_button:last input").val("");
            el_registry.setCheckNumber();

            check_number++;
            if(check_number > 1 && !$(".radio_number:last ~ .close").is("div")){
                $(".radio_number").last().after('<div class="button icon clear"><span class="material-icons">close</span></div>');
                $(".radio_button .clear").off("click").on("click", function (){
                    check_number--;
                    $(this).closest(".radio_button").remove();
                    el_registry.setCheckNumber();
                });
            }
        ...
        $("#prop_create [name=prop_name], #block_create [name=block_name]").on("blur", function (){
            el_tools.translateWithGoogle($(this).val(), "checkitems").
        ...
        $("#tbl_items tr.block_row i.material-icons").off("click").on("click", function (){
            let id = $(this).data("id"),
                icon = $(this).text(),
                $items = $("tr[data-parent = "+id+"]");
            if (icon === "expand_more"){
                $items.hide();
    ...
    setCheckNumber: function (){
        let $check_numbers = $(".radio_number");
