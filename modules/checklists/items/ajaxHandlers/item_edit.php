...
if(strlen(trim($_POST['field_types'])) == 0 && intval($_POST['is_block']) == 0){
    $err++;
    $errStr[] = 'Укажите тип поля';
    $errorFields[] = 'field_types';
}

$exist = $db->select('checkitems', ' where name = ? AND id <> ?', [$_POST['name'], $_POST['reg_id']]);
...
$exist_field = $db->select('checkitems', ' where field_name = ? AND id <> ?', [$_POST['field_name'], $_POST['reg_id']]); //print_r($exist_field);
...
if (is_array($_POST['option_label']) && count($_POST['option_label']) > 0 && strlen(trim($_POST['option_label'][0])) > 0) {
