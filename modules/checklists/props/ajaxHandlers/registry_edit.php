...
$exist = $db->select('checkprops', ' where name = ? AND id <> ?', [$_POST['name'], $_POST['reg_id']]);
...
$exist_field = $db->select('checkprops', ' where field_name = ? AND id <> ?', [$_POST['field_name'], $_POST['reg_id']]); //print_r($exist_field);
...
if (count($_POST['option_label']) > 0 && strlen(trim($_POST['option_label'][0])) > 0) {
...
if($err == 0) {
    ...
    $result = $db->update('checkprops', intval($_POST['reg_id']), $registry);
    if($result) {
