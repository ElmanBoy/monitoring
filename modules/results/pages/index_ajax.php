...
if (isset($_POST['params']) && substr_count($_POST['params'], 'id=') > 0) {
    include_once $_SERVER['DOCUMENT_ROOT'] . '/modules/results/pages/index_id_ajax.php';
} else {
$regId = 72;
        ...
        <table class="table_data" id="tbl_registry_items">
            ...
            foreach ($regs as $reg) {
                ...
                echo '<tr data-id="' . $reg->id . '" data-parent="' . $regId . '" tabindex="0">
                    ...
                    <td>' . $reg->comment . '</td>
                    <td class="link">'.
                    '<span class="material-icons reg_settings" data-id="'.$reg->id.'" title="Редактировать отчёт">edit</span>'.
                    '<span class="material-icons" title="Просмотр отчёта"><a href="/results/?id='.$reg->id.'">pageview</a></span></td>
                </tr>';
            }
            ?>
            </tbody>
