...
?>
<script src="/js/assets/frappe-gantt/dist/frappe-gantt.css?v=<?= $gui->genpass() ?>"></script>
...
<div class="scroll_wrap">
    <ul class='breadcrumb'>
        ...
        <li><a href='/plans?id=<?=$regId?>'><?=$table->short?></a></li>
    </ul>
		...
		<table class="table_data" id="tbl_registry_items">
			...
			foreach ($regs as $reg) {
				...
				echo '<tr data-id="' . $reg->id . '" data-parent="' . $regId . '" tabindex="0">
                    ...
                    <td>' . $reg->id . '</td>
                    <td class="status">
                    <span class="material-icons">' . $icon . '</span> '.$status.'</td>
                    <td>' . $parents['array'][$reg->parent_registry] . $itemStr . '</td>
                    <td class="group">' . stripslashes($reg->name) . (($auth->haveUserRole(3) || $auth->haveUserRole(1)) && $aCount > 0 ?
