...
$parent_item = $db->selectOne('institutions', 'where parent=' . $regId . ' LIMIT 1');
...
<div class="nav">
    <div class="nav_01">
        ...
        echo $gui->buildTopNav([
                'title' => 'Профили учреждений',
                //'registryList' => '',
                'renew' => 'Сбросить все фильтры',
                'create' => 'Новое учреждение',
                //'clone' => 'Копия записи',
                'delete' => 'Удалить учреждение',
                'reg_settings' => 'Нстройки реестра',
                'importEAIS' => 'Импорт из ЕАИС',
        ...
        <table class="table_data" id="tbl_registry_items">
            ...
            foreach ($regs as $reg) {
                ...
                echo '<tr data-id="' . $reg->id . '" data-parent="' . $regId . '" tabindex="0">
                    ...
                    <td>' . $reg->inn . $itemStr . '</td>
                    <td class="group">' . stripslashes($reg->short) . ' '. $reg->branchname . '</td>
                    <td>' . $reg->comment . '</td>
                </tr>';
            }
            ?>
...
<script src='/modules/institutions/js/registry.js?v=<?= $gui->genpass() ?>'></script>
<script src="/modules/institutions/js/registry_items.js?v=<?= $gui->genpass() ?>"></script>