...
if (isset($_POST['params']) && substr_count($_POST['params'], 'id=') > 0) {
	...
	$regs = $gui->getTableData('registry');
	?>
	<div class="nav">
		<div class="nav_01">
			<?
			...
			<table class="table_data" id="tbl_registry">
				...
				foreach ($regs as $reg) {
					...
					echo '<tr data-id="' . $reg->id . '" tabindex="0">
                                        ...
                                        <td class="link"><a href="/registry/?id=' . $reg->id . '">' . $reg->name . '</a></td>
                                        <td>' . $reg->comment . '</td>
                                        <td class="link">
                                            <span class="material-icons reg_settings" title="Настройки справочника"><a href="/registry">construction</a></span>
                                            <span class="material-icons" title="Просмотр справочника"><a href="/registry/?id=' . $reg->id . '">pageview</a></span>
                                        </td>
                                    </tr>';
				}
				?>
	...
	<script src="/modules/registry/js/registry.js?v=<?= $gui->genpass() ?>"></script>
	<?php
}
?>
