...
var el_signs_registry = {
    ...
    create_init: function(){
        ...
        $(document).on("content_load", function (){
            if (el_tools.function_exists("el_signs_registry.getAllPropsInCreateRegistry")) {
                el_signs_registry.getAllPropsInCreateRegistry();
            }
        });

        $("select[name=fromdb]").on("change", function (){
            el_signs_registry.showFieldsFromDB($(this).val(), $("input[name=selected_field]").val());
        });
        ...
        $(".pop_up_body .new_institution").off("click").on("click", function(e){
            e.preventDefault();
            el_signs_registry.cloneInstitution();
        });
        ...
        el_signs_registry.bindPassword();
        el_signs_registry.bindLoginAlias();
        el_signs_registry.getPropsInRegistry();
        el_signs_registry.bindDadata();
        el_app.sort_init();
        el_app.filter_init();
    },
        ...
        if ($items.length > 0) {
            ...
            $.post("/", {ajax: 1, action: "buildForm", props: newRegistryData}, function (data){
                ...
                $(".pop_up_body .new_section").off("click").on("click", function(e){
                    e.preventDefault();
                    el_signs_registry.cloneSection();
                });
                $("input[type=tel]").mask('+7 (999) 999-99-99');
                el_signs_registry.bindDadata();
                el_signs_registry.bindSetOrgByType($(".institutions:last"));
                el_signs_registry.bindSetMinistriesByOrg($(".institutions:last"));
                el_signs_registry.bindSetUnitsByOrg($(".institutions:last"));
                el_signs_registry.bindSetUserByUnit($(".institutions:last"));
                el_signs_registry.bindPassword();
                el_signs_registry.bindLoginAlias();
                el_signs_registry.bindTipsy();
                el_signs_registry.bindCalendar();
    ...
    cloneSection: function(){
        ...
        el_signs_registry.setStageNumber();
        ...
        el_signs_registry.bindTipsy();
        el_signs_registry.bindCalendar();
        el_signs_registry.scrollToLastInstitution();
    },
            ...
            $.post("/", {ajax: 1, action: "getUsersByOrg", orgId: $inst.val(), ministriesId: $ministries.val(),
                ...
                $users.html(data).trigger("chosen:updated").off("change").on("change", function (){
                    ...
                    $new_signer.off("click").on("click", function (e){
                        ...
                        el_signs_registry.setAgreementList(instanceObj);
                        $(this).hide();
