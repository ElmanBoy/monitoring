...
//Получение даты проведения проверки
...
$globalMin = $date->correctDateFormatFromMysql(min($allMinDates));
$globalMax = $date->correctDateFormatFromMysql(max($allMaxDates));
...
/*{{ institution }}<br>{{ institution_legal }}<br>тел/факс {{ institution_phones }}<br>{{ shedule_date }} № {{ shedule_number }}*/
//print_r($tmpl);
$html = '
        <style>
            @font-face {
...
<style>
    ...
    .schedule_tbl tr td, .schedule_tbl tr th{
        padding: 3px;
    }
    .schedule_tbl tr td input, .schedule_tbl tr td textarea,
    .schedule_tbl tr td .el_input, .schedule_tbl tr td .el_data{
        display: block;
    }
    textarea.el_input{
        min-height: 200px;
...
<div class='pop_up drag' style="width: 60vw; min-height: 70vh;">
    ...
    <div class='pop_up_body'>
        ...
        <div class="group">
            ...
            <table class="schedule_tbl">
                <tr>
                    <th>No п/п</th>
                    <th style="width:30%">Предложения по устранению выявленных нарушений и недостатков</th>
                    <th>Действия, необходимые для принятия мер по устранению нарушений</th>
                    <th>Сроки выполнения предложений (устранения нарушений)</th>
                    <th>Ответственный за выполнение</th>
                    <th></th>
                </tr>
                <?
                $check = $db->selectOne('checkstaff', ' WHERE id = ?', [$docId]);
                $tasks = $db->select('checkstaff', ' WHERE institution = ?', [5]);//заменить на автоопределение id учреждения
                $tids = [];
                if ($tasks) {
                ...
                <tr class="schedule_row">
                    ...
                    <td>
                        <div class='el_data'>
                            <input type='hidden' name='schedule_number[]' value='<?=$v['name']?>'>
                            <?=$v['name']?>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <div class='el_data'>
                            <textarea class='el_input' name='schedule_deadlines[]'></textarea>
                        </div>
                    </td>
                    <td>
                        <div class='el_data'>
                            <textarea class='el_input' name='schedule_responsible[]'></textarea>
                        </div>
                    </td>
                    <td>
                        <div class='button icon clear' style="display: none"><span class='material-icons'>close</span></div>
                    </td>
                </tr>
                <?
                        $num++;
                ...
                ?>
                    <tr class='schedule_row'>
                        <td>
                            <div class='el_data'>
                                <input type='text' class='el_input' name='schedule_number[]' value='1'>
                            </div>
                        </td>
                        <td>
                            <div class='el_data'>
                                <textarea class='el_input' name='schedule_offers[]'></textarea>
                            </div>
                        </td>
                        <td>
                            <div class='el_data'>
                        ...
                        <td>
                            <div class='button icon clear' style='display: none'><span
            ...
            <button class='button icon text new_schedule_row'>
                <span class='material-icons'>add</span>Еще строка
            </button>
        </div>
            <div class='confirm'>
    ...
    <script>
            ...
            $('.new_schedule_row').off('click').on('click', function (e) {
                ...
                $('.pop_up_body .schedule_row:last td:nth-child(2)')
                    .html("<textarea class='el_input' name='schedule_offers[]'></textarea>");
                $('.pop_up_body .schedule_row:last input, .pop_up_body .schedule_row:last textarea').val("");

                setScheduleRowsNumbers();
                $(".pop_up_body .schedule_row:last .clear").show().off("click").on("click", function(e){
                    e.preventDefault();
                    $(this).closest("tr").remove();
                    setScheduleRowsNumbers();
                });
                $('.pop_up').animate({
                    scrollTop: $('.pop_up').scrollTop() + $('.pop_up_body .schedule_row:last').position().top + 200
                }, 300);
            });

            function setScheduleRowsNumbers(){
                let $inputs = $('.pop_up_body .schedule_row');
                for(let i = 0; i < $inputs.length; i++){
                    $($inputs[i]).find("input[name='schedule_number[]']").val(i + 1);
                    $($inputs[i]).find("td:first").text(i + 1);
                }
            }
