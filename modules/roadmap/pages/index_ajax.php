...
$regs = $gui->getTableData($table->table_name, " AND (documentacial = 2 OR documentacial = 5)");
?>
<div class="nav">
    <div class="nav_01">
        <?
        echo $gui->buildTopNav([
            'title' => 'Акты и Графики устранения нарушени',
            //'registryList' => '',
        ...
        <table class="table_data" id="tbl_registry_items">
            ...
            foreach ($regs as $reg) {
                ...
                echo '<tr data-id="' . $reg->id . '" data-parent="' . $regId . '" tabindex="0">
                    ...
                    <td>' . $reg->comment . '</td>
                    <td class="link">';
                    if(intval($reg->act_agree) > 0) {
                        echo '<span class="material-icons addRoad" data-id="'.$reg->id.'" data-ins="' . $reg->source_id . '" title="Создать график устранения нарушений">add_road</span>';
                    }
                    if($is_object) {
                        echo '<span class="material-icons agreementDoc" data-id="' . $reg->id . '" title="Направить возражения">call_missed</span> ';
                    }else{
                        echo '<span class="material-icons agreementDoc" data-id="' . $reg->id . '" title="Просмотр переписки">pageview</span> ';
                    }
                    echo '<span class="material-icons viewDoc" data-id="'.$reg->id.'" title="Ознаокмление с актом">picture_as_pdf</span>
                    </td>
                </tr>';
            }
            ?>
