...
if($err == 0) {
    ...
    if ($table->table_name === 'users' && isset($registry['login'])) {
        ...
        if ($existingUser) {
            error_log("[REGISTRY_CREATE] Login exists (id=" . $existingUser->id . "), generating unique login");
            ...
            do {
                ...
                error_log("[REGISTRY_CREATE] Trying login: " . $newLogin . " - " . ($existingUser ? 'taken' : 'available'));
                $counter++;
