...
<div class='pop_up drag ui-draggable' id='notificationsList'>
    ...
    <div class='pop_up_body' style='min-height: 27rem;'>
        <div id='notificationHeader'>
            ...
            ?>
            <span class="material-icons sound" id="notificationSound"
                  title="<?= $soundTitle ?>"><?= $soundIcon ?></span>
            <span class="material-icons clear" id="notificationDelete" title="Удалить все уведомления">delete</span>
        </div>
        <div class='group'>
            <?
            $nots = isset($_SESSION['user_id']) && intval($_SESSION['user_id']) > 0
                ? $notes->getRecordsToPanel(intval($_SESSION['user_id']))
                : ['messages' => '', 'countTotal' => 0, 'countUnseen' => 0];
            echo $nots['messages'];
            ?>
        </div>
    </div>
