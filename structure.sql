...
create index if not exists idx_cam_transactions_row
	on cam_transactions (table_name, row_id);

create index if not exists idx_cam_transactions_user
	on cam_transactions (user_id);
...
(
	...
	reg_id integer not null
		constraint fk_regfields_registry
			references cam_registry
				on delete cascade,
	prop_id integer not null
		constraint fk_regfields_prop
			references cam_regprops
				on delete cascade,
	sort integer,
	label text not null,
	required integer,
...
(
	...
	author integer,
	reg_id integer not null
		constraint fk_checkfields_checklist
			references cam_checklists
				on delete cascade,
	prop_id integer not null
		constraint fk_checkfields_prop
			references cam_checkitems
				on delete cascade,
	sort integer,
	label text not null,
	required integer,
...
create index if not exists idx_cam_tasks_author
	on cam_tasks (author);
...
(
	...
	name text,
	institution integer
		constraint fk_ousr_institution
			references cam_institutions
				on delete cascade
);
...
(
	...
	author integer,
	institution integer
		constraint fk_checkinstitutions_institution
			references cam_institutions
				on delete restrict,
	check_types integer,
	units integer,
...
create index if not exists idx_cam_checkinstitutions_plan
	on cam_checkinstitutions (plan_uid, plan_version);

create index if not exists idx_cam_checkinstitutions_institution
	on cam_checkinstitutions (institution);
...
(
	...
	name text,
	institution integer
		constraint fk_ministries_institution
			references cam_institutions
				on delete cascade
);
	...
	institution integer
		constraint fk_units_institution
			references cam_institutions
				on delete cascade,
	ministries integer
		constraint fk_units_ministries
			references cam_ministries
				on delete set null
);
...
(
	...
	phones text,
	ministries integer
		constraint fk_users_ministries
			references cam_ministries
				on delete set null,
	archived_by integer
		constraint cam_users_archived_by_fkey
...
create unique index if not exists idx_cam_users_login
	on cam_users (login);

create index if not exists idx_cam_users_institution
	on cam_users (institution);

create index if not exists idx_cam_users_active
	on cam_users (active);

create index if not exists idx_cam_users_roles_gin
	on cam_users using gin (roles);
	...
	user_id integer not null
		constraint fk_usersettings_user
			references cam_users
				on delete cascade,
	module_id integer
		constraint fk_usersettings_module
			references cam_modules
				on delete cascade,
...
create index if not exists idx_cam_units_institution
	on cam_units (institution);
...
(
	...
	inspections integer,
	archived_by integer
		constraint cam_checksplans_archived_by_fkey
			references cam_users
				on delete set null,
	archived_at timestamp
);
...
create index if not exists idx_cam_checksplans_active
	on cam_checksplans (active);

create index if not exists idx_cam_checksplans_year
	on cam_checksplans (year);
...
(
	...
	check_uid text,
	"user" integer
		constraint fk_checkstaff_user
			references cam_users
				on delete restrict,
	dates text,
	institution integer
		constraint fk_checkstaff_institution
			references cam_institutions
				on delete restrict,
	task_id integer,
	unit integer,
	...
	is_head integer,
	ministry integer
		constraint fk_checkstaff_ministry
			references cam_ministries
				on delete set null,
	order_number text,
	order_date date,
...
create index if not exists idx_cam_checkstaff_institution
	on cam_checkstaff (institution);

create index if not exists idx_cam_checkstaff_user
	on cam_checkstaff ("user");

create index if not exists idx_cam_checkstaff_active
	on cam_checkstaff (active);

create index if not exists idx_cam_checkstaff_order_id
	on cam_checkstaff (order_id);
...
create index if not exists idx_cam_notifications_user_viewed
	on cam_notifications (user_id, viewed);

create index if not exists idx_cam_notifications_user_created
	on cam_notifications (user_id asc, created_at desc);
...
(
	...
	author integer,
	user_id integer
		constraint fk_checksresult_user
			references cam_users
				on delete restrict,
	task_id integer,
	checklist integer,
...
create index if not exists idx_cam_checksresult_task
	on cam_checksresult (task_id);

create index if not exists idx_cam_checksresult_checklist
	on cam_checksresult (checklist);
...
(
	...
	created_at timestamp default CURRENT_TIMESTAMP,
	author integer not null
		constraint fk_agreement_author
			references cam_users
				on delete restrict,
	active integer default 1,
	name text,
	...
	brief text,
	initiator integer
		constraint fk_agreement_initiator
			references cam_users
				on delete set null,
	initiation timestamp,
	agreementlist jsonb,
	...
	executors_head integer
		constraint fk_agreement_executors_head
			references cam_users
				on delete set null,
	plan_id integer,
	ins_id integer,
	...
	archived_by integer
		constraint cam_agreement_archived_by_fkey
			references cam_users
				on delete set null,
	archived_at timestamp
);
...
(
	...
	doc_id integer not null
		constraint fk_signs_agreement
			references cam_agreement
				on delete cascade
...
create index if not exists idx_cam_signs_doc
	on cam_signs (table_name, doc_id);

create index if not exists idx_cam_signs_user_doc
	on cam_signs (user_id, doc_id, table_name);

create index if not exists idx_cam_agreement_status
	on cam_agreement (status);

create index if not exists idx_cam_agreement_source
	on cam_agreement (source_table, source_id);

create index if not exists idx_cam_agreement_initiator
	on cam_agreement (initiator);

create index if not exists idx_cam_agreement_documentacial
	on cam_agreement (documentacial);

create index if not exists idx_cam_agreement_created_at
	on cam_agreement (created_at desc);

create index if not exists idx_cam_ministries_institution
	on cam_ministries (institution);

create index if not exists index_foreignkey_cam_institutions_eais
	on cam_institutions (eais_id);

create index if not exists idx_cam_institutions_active
	on cam_institutions (active);

create index if not exists idx_cam_institutions_inn
	on cam_institutions (inn);

create index if not exists idx_cam_institutions_superior
	on cam_institutions (superior);
...
(
	...
	created_at timestamp default CURRENT_DATE,
	author integer not null
		constraint fk_reminders_author
			references cam_users
				on delete cascade,
	active integer default 1,
	comment text,
	employee integer
		constraint fk_reminders_employee
			references cam_users
				on delete cascade,
	datetime timestamp,
	task_id integer,
...
create index if not exists idx_cam_reminders_employee_datetime
	on cam_reminders (employee, datetime)
	where (active = 1);
...
create index if not exists idx_cam_tasklog_module_action
	on cam_tasklog (module, action);

create index if not exists idx_cam_tasklog_author
	on cam_tasklog (author);
...
create index if not exists idx_cam_files_userid
	on cam_files (userid);
...
(
	...
	name text,
	tasks integer
		constraint fk_checksviolations_task
			references cam_tasks
				on delete cascade,
	violations integer,
	checklist integer,
	objections text
...
create index if not exists idx_cam_checksviolations_checklist
	on cam_checksviolations (checklist);
