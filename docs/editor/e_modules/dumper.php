...
define('DBNAMES', $database_dbconn);
...
class dumper {
	...
	function backup() {
		...
		$buttons = "<A ID=save HREF='' STYLE='display: none;'>Скачать файл</A> &nbsp; <INPUT class=but ID=back TYPE=button VALUE='Вернуться' DISABLED onClick=\"history.back();\">";
	...
	function restore(){
		if (!isset($_POST)) {$this->main();}
		set_error_handler("SXD_errorHandler");
		$buttons = "<INPUT class=but ID=back TYPE=button VALUE='Вернуться' DISABLED onClick=\"history.back();\">";
	...
	function main(){
		...
		$buttons = "<INPUT TYPE=submit class=but VALUE=Применить>&nbsp;&nbsp;<INPUT class=but TYPE=button VALUE=Выход onClick=\"location.href = 'dbserv.php'\">";
...
<BODY>
<h4 align=center>Создание и восстановление резервной копии базы данных</h4>
...
<TD WIDTH=59%><INPUT NAME=login TYPE=text CLASS=text value="$username_dbconn"></TD>
</TR>
<TR>
<TD>Пароль:</TD>
<TD><INPUT NAME=pass TYPE=password CLASS=text value="$password_dbconn"></TD>
</TR>
</TABLE><script language="javascript">skb.submit();</script>
</FIELDSET>
</SPAN>
<SCRIPT>
